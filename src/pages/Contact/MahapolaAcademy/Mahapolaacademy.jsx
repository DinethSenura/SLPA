...
const FetchDataPage = () => {
  ...
  const requestData = useMemo(() => ({
    data: [
      {
        article_menu: 'Mahapola Academy',
        article_code: 'TWFoYXBvbGEgQWNhZGVteQ==',
      ...
      {data && data.status && data.data && data.data.article_info ? (
        <div>
          ...
          {data.data.article_info.image && (
              <img
              src={`https://www.slpa.lk/application_resources/images/${data.data.article_info.image}`}
              alt={data.data.article_info.title}
              style={{ width: '100%', height:'350px', marginTop:'70px', marginBottom: '10px' }}
            />
            )}
